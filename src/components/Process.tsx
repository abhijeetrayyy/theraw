...
export default function Process() {
  ...
  useGSAP(() => {
    ...
    // LAYER 5: Step entrance with multi-layer reveals
            ...
            <div className="space-y-0">
              {steps.map((step, i) => (
                <div key={i} className="proc-step relative pl-0 md:pl-24 group cursor-default">
                  <div className="proc-dot hidden md:block absolute left-[23px] top-16 w-[15px] h-[15px] rounded-full border-2 border-accent bg-bg z-10 transition-all duration-400" />
                  <div style={{ paddingTop: "clamp(4rem, 8vw, 8rem)", paddingBottom: "clamp(4rem, 8vw, 8rem)", borderBottom: i < steps.length - 1 ? "1px solid var(--color-text-08)" : "none" }} className="group-hover:bg-bg-2/30 -mx-4 px-4 rounded-lg transition-colors duration-400">
                    <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: "clamp(3rem, 6vw, 6rem)" }}>
                      <div className="proc-img relative w-full aspect-[4/3] overflow-hidden rounded-lg">
